#Author          :LiteSpeedtech & Matan Schatzman & GiaTuan
#date            :20210423
#version         :1.8.1
...
function crawlreq() {
    ...
    if [ "${CHECKMATCH}" == '' ]; then
        CHECKMATCH=$(grep -Eio '(swift-performance: miss|swift-performance: hit|swift-performance: no-cache)'\
        <<< ${CURLRESULT} | tr -d '\n')
    fi
    if [ "${CHECKMATCH}" == '' ]; then
    ...
    case ${CHECKMATCH} in
        'CreatedSet-CookieSet-CookieSet-Cookie'|swift-performance:\ miss)
            echoY 'Đang tạo cache cho trang web'
            cachecount 'miss'
            protect_count 0
        ;;
        ...
        ;;        
        swift-performance-Cache-Control:\ no-cache)
            echoY 'No Cache page'
            cachecount 'no'
            protect_count 1
